...
class BaseAgent:
    ...
    def _compute_obs(self):
        ...
        compass = torch.tensor(
            [rot["y"] / 360.0, agent_meta.get("cameraHorizon", 0.0) / 360.0],
            dtype=torch.float32,
        )

        action_id = self.action2id[self.current_action]
        current_action = F.one_hot(
            torch.tensor(action_id), num_classes=len(self.action_list)
        ).float()
        res_level = torch.tensor([float(self.current_downgrade)], dtype=torch.float32)
        remaining_budget = torch.tensor(
            [float(self.remaining_sensing_budget)], dtype=torch.float32
        )

        obs = torch.cat(
            [vis_feat, gps, compass, current_action, res_level, remaining_budget], dim=0
        )
        return obs
